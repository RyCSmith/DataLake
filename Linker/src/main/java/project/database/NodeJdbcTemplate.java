...
public class NodeJdbcTemplate {
	...
	public List<Node> getNodesByWord(String word) {
		String SQL = 
				"select * from node_table where node_id in (" +
					"select node_id from ii_table where word = ?)";
		
		List<Node> nodes = jdbcTemplateObject.query(SQL, new Object[]{word}, new NodeMapper());
		return nodes;
	}
